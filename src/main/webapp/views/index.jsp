...
<body>
	...
	<!-- 사용자 보유 권한 - sessionScope.auththory(배열) -->
	<!-- 사용자 협업 - sessionScope.coorList(배열) -->
	<!-- ======= Header ======= -->
		...
		<nav class="header-nav ms-auto">
			<ul class="d-flex align-items-center">
				...
				<li class="nav-item dropdown"><a class="nav-link nav-icon"
					...
					<ul
						...
						<li class="dropdown-header"><c:if
								test="${sessionScope.userType eq  'emp'}">${sessionScope.loginInfo.emp_name }
                  </c:if> <c:if test="${sessionScope.userType eq  'store'}">${sessionScope.loginInfo.store_name }
                  </c:if> 님의 알림 리스트</li>
						<li>
				...
				<li class="nav-item dropdown pe-3"><a
					...
					onclick="" data-bs-toggle="dropdown"> <c:if
						...
						</c:if> <span class="d-none d-md-block dropdown-toggle ps-2"> <c:if
								test="${sessionScope.userType eq  'emp'}">
                  ${sessionScope.loginInfo.emp_name }
                  </c:if> <c:if test="${sessionScope.userType eq  'store'}">
		...
		<ul class="sidebar-nav" id="sidebar-nav">
			...
			<li class="nav-item"><a class="nav-link collapsed"
				...
				<div
					...
					<h3>
						<c:if test="${sessionScope.userType eq  'emp'}">${sessionScope.loginInfo.pos_name }
                  </c:if>
			...
			<li class="nav-item"><a class="nav-link collapsed"
				...
				<ul id="sign-nav" class="nav-content collapse "
					...
					<li><a onclick="location.href='docRec.go'"> <i
							class="bi bi-circle"></i><span>결재 수신 문서함</span>
					</a></li>
					<li><a onclick="location.href='docDis.go'"> <i
			...
			<li class="nav-item"><a class="nav-link collapsed"
				onclick="location.href='OrgChart.go'"> <i
					class="bi bi-chat-left-text"></i> <span>조직도</span>
			</a></li>
			<!-- 조직도 Nav -->
			...
			<li class="nav-item"><a class="nav-link collapsed"
				...
				<ul id="floor-nav" class="nav-content collapse "
					data-bs-parent="#sidebar-nav">
					<li><a onclick="location.href='floor.go'"> <i
			...
			<li class="nav-item"><a class="nav-link collapsed"
				onclick="location.href='teamSch.go'"> <i
					class="bi bi-chat-left-text"></i> <span>팀일정</span>
			</a></li>
			<!-- 팀 일정 Nav -->

			<li class="nav-item"><a class="nav-link collapsed"
				onclick="location.href='facList.go'"> <i
					class="bi bi-chat-left-text"></i> <span>시설예약</span>
			</a></li>
			<!-- 시설예약 Nav -->
